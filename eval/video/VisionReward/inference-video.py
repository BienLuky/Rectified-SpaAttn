...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', required=True, default=None)

    args = parser.parse_args()

    json_score = {}
    json_score["vr_mean_score"] = 0
    scores = []
    sub_json = []
    ...
    for dirpath, _, filenames in os.walk(args.path):
        for f in filenames:
            if f.lower().endswith(".mp4"):
                score_dir = {}
                video1 = os.path.join(dirpath, f)
                prompt = f[:-6]
                prompt_score = score(video1, prompt)
                score_dir["prompt"] = prompt
                score_dir["vr_score"] = prompt_score
                scores.append(prompt_score)
                sub_json.append(score_dir)

    vr_mean_score = np.mean(scores)
    json_score["vr_mean_score"] = vr_mean_score
    json_score["vr_sub_score"] = sub_json

    base_dir = os.path.dirname(os.path.abspath(__file__))
    json_path = os.path.join(base_dir, os.path.basename(os.path.normpath(args.path))+".json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(json_score, f, ensure_ascii=False, indent=4)

    print(f"save vr score to {json_path}")
