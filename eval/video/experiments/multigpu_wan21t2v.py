...
from rectified_spaattn.rectified_wan21_attn import RectifiedWanT2VSpaAttnProcessor2_0
...
from generation import read_prompt_list, generate_func_wan21t2v
...
def teacache_forward(
    ...
    for name, processor in get_attn_processors(self.blocks).items():
        if isinstance(processor, RectifiedWanT2VSpaAttnProcessor2_0):
...
def worker(device_id, prompts, args, sample_path):
    ...
    height, width, frame = 720, 1280, 81
    ...
    first_frame_blocks = math.ceil(img_block_num // latent_time_)
    ...
    pipe.transformer.__class__.teacache_thresh = args.teacache_thresh # 0.1 for 1.6x speedup, 0.15 for 2.1x speedup
    ...
    for k,v in get_attn_processors(pipe.transformer).items():
        if "attn1" in k:
            attn_processors[k] = RectifiedWanT2VSpaAttnProcessor2_0(args.mode, select_block_num, hilbert_params[0][2], p_remain_rates, processors_id, first_frame_blocks)
        elif "attn2" in k:
            attn_processors[k] = RectifiedWanT2VSpaAttnProcessor2_0("flash", select_block_num, hilbert_params[0][2], p_remain_rates, processors_id)
            processors_id += 1   
    set_attn_processor(pipe.transformer, attn_processors)

    # generate videos
    generate_func_wan21t2v(pipe, prompts, sample_path, loop=1, kwargs=kwargs)
