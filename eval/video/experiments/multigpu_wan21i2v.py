...
from generation import read_prompt_list, read_image_list, generate_func_wan21i2v
...
def worker(device_id, prompts, images, args, sample_path):
    ...
    # generate videos
    generate_func_wan21i2v(pipe, prompts, images, sample_path, loop=1, kwargs=kwargs)
...
if __name__ == "__main__":
    ...
    prompt_path = os.path.join(base_dir, "../vbench/sampled_images_600.json")
    prompt_list = read_prompt_list(prompt_path)
    image_list = read_image_list(prompt_path)
    ...
    images = [image_list[i::num_gpus] for i in range(num_gpus)]

    with ProcessPoolExecutor(max_workers=num_gpus) as executor:
        futures = []
        for device_id, chunk in enumerate(chunks):
            futures.append(executor.submit(worker, device_id, chunk, images[device_id], args, sample_path))
