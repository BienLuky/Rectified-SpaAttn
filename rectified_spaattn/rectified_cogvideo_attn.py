...
def block_sparse_attention_combined(
    ...
    seqlens = torch.tensor([context_size] * batch_size, dtype=torch.int32).to(device=query.device)
    ...
    normal_blocks = num_blocks - ((text_length+pad) // block_size_M)
    text_end_block = num_blocks
    ...
    value_text = value
    # use Flash Attention
    cu_seqlens_q_text = torch.tensor([0, text_length, text_length * batch_size], dtype=torch.int32, device=query.device)
    max_seqlen_q_text = query_text.shape[2]
...
class RectifiedCogVideoXVideoSpaAttnProcessor2_0:
    def __init__(self, 
            mode, select_block_num, block_neighbor_list, p_remain_rates, processor_id=0):
        self.mode = mode
    ...
    def __call__(
        ...
        cu_seqlens_q = torch.tensor([0, S_q, S_q * B], dtype=torch.int32, device=query.device)
        cu_seqlens_kv = torch.tensor([0, s_k, S_k * B], dtype=torch.int32, device=query.device)
        max_seqlen_q, max_seqlen_kv = S_q, S_k
        if self.mode == "sparse" and self.current_step >= 5: 
