...
def block_sparse_attention_combined(
    ...
    # Non-participating key–value pairs are zeroed out to eliminate their influence.
    key.masked_fill_(~(attn_mask.transpose(-2, -1)), 0.0)
    value.masked_fill_(~(attn_mask.transpose(-2, -1)), 0.0)

    batch_size, num_heads, context_size, head_dim = query.shape
    
    # Process variable-length sequences and compute the effective sequence length of valid text tokens (attenable).
    if cu_seqlens_q is not None and cu_seqlens_kv is not None:
        seqlens = cu_seqlens_q[1:2]
        attenable = 256 - (cu_seqlens_q[2] - seqlens)
    ...
    normal_blocks = num_blocks - (256 // block_size_M)
...
class RectifiedHunyuanVideoSpaAttnProcessor2_0:
    def __init__(self, mode, select_block_num, block_neighbor_list, p_remain_rates, processor_id=0):
        self.mode = mode
    ...
    def __call__(
        ...
        # 5. Attention
        B, H, S, D = query.shape
        num_true = attention_mask.sum().item()
        cu_seqlens_q = torch.tensor([0, num_true, S], dtype=torch.int32, device=query.device)
        cu_seqlens_kv = cu_seqlens_q
        max_seqlen_q, max_seqlen_kv = S, S
        if self.mode == "sparse": 
